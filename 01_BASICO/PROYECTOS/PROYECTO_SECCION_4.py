...
imprimir:
-nombre de usuario
-avisar que se ha pensado en un numero entre 1-100 y tiene 8 intentos
para adivinar
...
def adivina_numero():
    '''
    funcion para iniciar juego "ADIVINA EL NUMERO"
    '''
    os.system("cls")
    ...
    lista_intentos = []
    os.system("cls")
    print(f"hola {usuario}, intenta adivinar el numero secreto (entre "
          "{numero_menor} y {numero_mayor}), tienes 8 intentos")
    while vidas_inicial > 0:
        valor_intento = int(input("ingresa un numero: "))
        if valor_intento in lista_intentos:
            print(f"ya intentaste con el numero {valor_intento}, selecciona "
                  "otro numero")
        else:
            lista_intentos.append(valor_intento)
            if valor_intento < numero_menor or valor_intento > numero_mayor:
                print(f"el numero ingresado '{valor_intento}' esta fuera "
                      "del rango")
            else:
                if valor_intento < numero_secreto:
                    print(f"el numero ingresado '{valor_intento}' es menor al "
                          "numero secreto, te quedan {vidas_inicial-1}"
                          " intentos")
                    vidas_inicial -= 1
                    total_intentos += 1

                elif valor_intento > numero_secreto:
                    print(f"el numero ingresado '{valor_intento}' es mayor al "
                          "numero secreto, te quedan {vidas_inicial-1} "
                          "intentos")
                    vidas_inicial -= 1
                    total_intentos += 1
                    ...
                    total_intentos += 1
                    os.system("cls")
                    print(f"felicidades {usuario}, el numero secreto era '"
                          "{valor_intento}', y lo descubriste en "
                          "{total_intentos} intentos")
                    break
    else:
        os.system("cls")
        print(f"{usuario} no descubriste el numero secreto '{numero_secreto}',"
