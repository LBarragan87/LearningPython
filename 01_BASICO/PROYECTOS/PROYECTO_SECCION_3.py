...
def analizador_texto():
    ...
    for letra in lista_letras:
        print(f"({y}) la letra {letra} aparece "
              "{texto_mayusculas.count(letra)} veces.")
        y += 1
    ...
    if confirmar_existencia_palabra in texto_mayusculas:
        print(f"la palabra '{confirmar_existencia_palabra}' "
              "SI aparece en el texto")
    else:
        print(f"la palabra '{confirmar_existencia_palabra}' "
